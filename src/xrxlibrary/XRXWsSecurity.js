 ...
 * 				09/22/2017 TC		Updated addWsSecurity() to support
 *									plain text password.
 *				05/21/2018 TC		Updated comments.
 *				01/03/2019 TC		Updated </env:Header> to </soap:Header>.
 * 				01/04/2019 TC       Added addWsSecurityHeader() and getWsSecurityHeader().
 *									Deprecated addWsSecurity() and getSecurityHeader().
 *				06/20/2019 TC 		Updated getWsSecurityHeader() and getSecurityHeader() 
...
*/
function addWsSecurityHeader( envelope, username, password, type )
{
	if ((username == null) || (username == undefined) || (password == null) || (password == undefined)) {
		return envelope;
	} else {
		return envelope.replace( '</soap:Header>', getWsSecurityHeader( username, password, type ) + '</soap:Header>' );
	}
}
...
* @param	{string}	[type="PasswordDigest"]		Ws-Security password type. Supported values are 
*									"PasswordDigest" and "PasswordText".
* @return	{string}	Ws-Security header.
*/
function getWsSecurityHeader( username, password, type )
{
	var XRX_SECURITY_HEADER = '<wsse:Security soap:mustUnderstand="1"><wsu:Timestamp wsu:Id="[TimestampId]"><wsu:Created>[Created]</wsu:Created>'
    + '<wsu:Expires>[Expires]</wsu:Expires></wsu:Timestamp>'
	+ '<wsse:UsernameToken wsu:Id="[SecurityId]"><wsse:Username>[Username]</wsse:Username>[wsse:Password]'
    + '<wsse:Nonce>[Nonce]</wsse:Nonce><wsu:Created>[Created2]</wsu:Created></wsse:UsernameToken></wsse:Security>';
	
    var result = XRX_SECURITY_HEADER;
    
    var myDate = new Date();	// Get current time at the local time zone
	var utcMillis = myDate.getTime() + ( myDate.getTimezoneOffset() * 60000 );		// Convert it to UTC time in milliseconds
	myDate.setTime( utcMillis );
	
    var expDate = new Date();  
	expDate.setTime( utcMillis + 60*60000 );   // Set the expiration time to current time plus 1 hour
	...
	if ( type === "PasswordText" ) {
		var pwText = '<wsse:Password Type="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText">' + password + '</wsse:Password>';
		result = result.replace( '[wsse:Password]', pwText );
	} else {
		//Generate digest
		var digest = generatePasswordDigest( nonce, created, password );
		var digestStr = '<wsse:Password Type="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordDigest">' + digest + '</wsse:Password>';
		result = result.replace( "[wsse:Password]", digestStr );
	}
    return result;
}
...
* @return	{string}	soap request contains Ws-Security header.
*/
function addWsSecurity( envelope, username, password, action, to, type )
{
    return envelope.replace( '</soap:Header>', getSecurityHeader( username, password, action, to, type ) + '</soap:Header>' );
}
...
* @return	{string}	Ws-Security header.
*/
function getSecurityHeader( username, password, action, to, type )
{
    ...
    + '<wsse:Security soap:mustUnderstand="1"><wsu:Timestamp wsu:Id="[TimestampId]"><wsu:Created>[Created]</wsu:Created>'
    ...
    + ' wsu:Id="[SecurityId]"><wsse:Username>[Username]</wsse:Username>[wsse:Password]'
    + '<wsse:Nonce>[Nonce]</wsse:Nonce><wsu:Created>[Created2]</wsu:Created></wsse:UsernameToken></wsse:Security>';
...
* @param {string}	created             time in proper format
...
*   
* @private
* @param {object}	date        Date object with current time
* @return {string}	nonce
*/
...
*   
* @private
* @param {object}	d1          current Date object
